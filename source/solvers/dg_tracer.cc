...
DGTracer<dim>::assemble_system_matrix()
{
                  ...
                  if (velocity_dot_n > 0)
                    {
                      local_matrix(i, j) +=
                        jump_i // [\phi_i]
                        * scratch_data.face_phi_outflow[q][j] * velocity_dot_n *
                        JxW;
                    }
                  else
                    {
  ...
  template <int dim>
  void DGTracer<dim>::assemble_system_rhs()
  {
  ...
  template <int dim>
  void DGTracer<dim>::attach_solution_to_output(DataOut<dim> & data_out)
  {
    data_out.add_data_vector(dof_handler, present_solution, "dg_tracer");
  }

  template <int dim>
  double DGTracer<dim>::calculate_L2_error()
  {
  ...
  template <int dim>
  void DGTracer<dim>::finish_simulation()
  {
    auto         mpi_communicator = triangulation->get_communicator();
  ...
  template <int dim>
  void DGTracer<dim>::percolate_time_vectors()
  {
  ...
  template <int dim>
  void DGTracer<dim>::finish_time_step()
  {
    percolate_time_vectors();
  }

  template <int dim>
  void DGTracer<dim>::postprocess(bool first_iteration)
  {
  ...
  template <int dim>
  void DGTracer<dim>::calculate_tracer_statistics()
  {
    auto mpi_communicator = triangulation->get_communicator();
            ...
            for (unsigned int q = 0; q < n_q_points; q++)
              {
                ...
                max_tracer_value =
                  std::max(q_tracer_values[q], max_tracer_value);
                min_tracer_value =
                  std::min(q_tracer_values[q], min_tracer_value);
              }
          }
      }
    ...
    variance_integral =
      Utilities::MPI::sum(variance_integral, mpi_communicator);
  ...
  template <int dim>
  void DGTracer<dim>::write_tracer_statistics()
  {
    auto mpi_communicator = triangulation->get_communicator();
  ...
  template <int dim>
  void DGTracer<dim>::pre_mesh_adaptation()
  {
  ...
  template <int dim>
  void DGTracer<dim>::post_mesh_adaptation()
  {
    auto mpi_communicator = triangulation->get_communicator();
  ...
  template <int dim>
  void DGTracer<dim>::write_checkpoint()
  {
  ...
  template <int dim>
  void DGTracer<dim>::read_checkpoint()
  {
    auto mpi_communicator = triangulation->get_communicator();
  ...
  template <int dim>
  void DGTracer<dim>::set_initial_conditions()
  {
  ...
  template <int dim>
  void DGTracer<dim>::solve_linear_system(const bool initial_step,
                                          const bool /*renewed_matrix*/)
  {
    ...
    const double ilu_fill =
      simulation_parameters.linear_solver.ilu_precond_fill;
    const double ilu_atol =
      simulation_parameters.linear_solver.ilu_precond_atol;
    const double ilu_rtol =
      simulation_parameters.linear_solver.ilu_precond_rtol;
