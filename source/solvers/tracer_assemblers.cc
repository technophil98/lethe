...
template <int dim>
void
DGTracerAssemblerCore<dim>::assemble_matrix(
  DGTracerScratchData<dim> & scratch_data,
  StabilizedMethodsCopyData &copy_data)
{
  // Scheme and physical properties
  const double diffusivity = this->physical_properties.tracer_diffusivity;
  const auto   method      = this->simulation_control->get_assembly_method();
...
template <int dim>
void
DGTracerAssemblerCore<dim>::assemble_rhs(DGTracerScratchData<dim> &scratch_data,
                                         StabilizedMethodsCopyData &copy_data)
{
  // Scheme and physical properties
  const double diffusivity = this->physical_properties.tracer_diffusivity;
...
template class DGTracerAssemblerCore<2>;
template class DGTracerAssemblerCore<3>;
